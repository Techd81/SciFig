...
| `computer_ai_ml` | model, algorithm, epoch, train/test split, loss, validation loss, accuracy, R2/RMSE/MAE/AUC/F1, SHAP, feature importance, residuals | `training_curve`, `grouped_bar`, `scatter_regression`, `residual_vs_fitted`, `line`, `lollipop_horizontal` | `dotplot`, `heatmap_annotated`, `roc`, `pr_curve`, `calibration`, `confusion_matrix` | Training dynamics, cross-validation summaries, residual diagnostics, ROC/PR, calibration |
