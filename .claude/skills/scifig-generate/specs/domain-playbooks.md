...
- Classifier errors: `rf_classifier_report_board` first when RF feature importance is also available; otherwise `classifier_validation_board`, then `confusion_matrix` when only true/predicted labels are present
