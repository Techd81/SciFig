...
- For wide metric inputs, reserve right-side benchmark whitespace and render a compact metric-table sidecar for the top model so AUC/F1/precision/recall remain visible without becoming legend entries; if the rendered sidecar touches bar data, auto-relocate through the helper priority slots, then fall back to an outside metric box before failing QA.
