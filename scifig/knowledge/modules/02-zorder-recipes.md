...
The `draw_gradient_box` trick uses `ax.imshow` of a 1-pixel-wide RGBA matrix to fake a gradient — see `knowledge/techniques/gradient-box.md`.
...
`runtime/helpers.py` exposes the recipes as callable applicators that rewrite zorder-naive generator output:
