...
`runtime/helpers.py` exposes the kernel as a single function. Phase 3 code generators MUST call it before constructing any figure.
...
Frequency counts are from `knowledge/scripts/stats.json`. Re-run `knowledge/scripts/extract.py` and `knowledge/scripts/enrich.py` to refresh.
