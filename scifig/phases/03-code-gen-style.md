...
> Generator implementations are in sub-files under `runtime/`:
> Generator implementations are domain-split legal Python modules under `runtime/` named `generators_<domain>.py`:
...
Phase 3 must consult `knowledge/` (the 7-module knowledge base built from the current 94-case 顶刊复刻 corpus) before emitting chart code. Operational helpers live in `runtime/template_mining_helpers.py`.
...
Palette defaults are defined in `resources/template-palette-registry.json`.
...
`runtime/registry.py::CHART_GENERATORS`. Phase-3 dispatch reads this
dict directly; do not duplicate the registry in this document.

```python
registry_source = Path("scifig/runtime/registry.py").read_text(encoding="utf-8").strip()
...
The canonical helper runtime is `runtime/helpers.py`. Do not duplicate helper implementations in this phase document. Step 3.6 must read that file, embed it into `full_code_string`, execute it, and use its public contract:
...
If a helper behavior needs to change, edit `runtime/helpers.py` and update tests; do not paste a local replacement here.
### Step 3.4b: Improved Multi-panel Composition

Use `panelBlueprint` as the source of truth and embed the canonical source from `runtime/generators-multipanel.py` during Step 3.6. Do not duplicate `resolve_canvas`, `resolve_panel_geometry`, or `compose_multipanel` here.
...
GENERATOR_SOURCE_FILES = [
    "scifig/runtime/generators_clinical.py",
    "scifig/runtime/generators_composition.py",
    "scifig/runtime/generators_diagnostics.py",
    "scifig/runtime/generators_distribution.py",
    "scifig/runtime/generators_engineering.py",
    "scifig/runtime/generators_genomics.py",
    "scifig/runtime/generators_matrix.py",
    "scifig/runtime/generators_ml.py",
    "scifig/runtime/generators_network.py",
    "scifig/runtime/generators_radar.py",
    "scifig/runtime/generators_ranked.py",
    "scifig/runtime/generators_scatter.py",
    "scifig/runtime/generators_time_series.py",
]
...
helper_source = Path("scifig/runtime/helpers.py").read_text(encoding="utf-8").strip()
helper_source = helper_source.replace("```python", "", 1).rsplit("```", 1)[0].strip()
multipanel_source = Path("scifig/runtime/generators-multipanel.py").read_text(encoding="utf-8").strip()
...
template_mining_helpers_source = Path("scifig/runtime/template_mining_helpers.py").read_text(encoding="utf-8").strip()
...
_FONTS_DIR_PATH = Path("scifig/assets/fonts").resolve()
