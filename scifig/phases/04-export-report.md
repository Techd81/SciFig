...
If automatic layout passes are insufficient but the figure is otherwise correct, hand-edit the canonical editable SVG (`output/figure1.editable.svg`) in Inkscape or Illustrator, save as `output/figure1.final.svg`, and regenerate PNG/PDF derivatives from that file. Update `output/reports/svg_render_qa.json` to record the manual edit and derivative paths.
