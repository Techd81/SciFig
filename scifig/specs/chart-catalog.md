...
Expanded chart taxonomy for `scifig`. Use this as the lookup table during Phase 2.

**Status**: the registry exposes 121 callable chart keys. Dedicated generator coverage is incremental; `generic_chart` fallback inside this skill is allowed only as a temporary compatibility layer and must not be counted as template-level implementation.
