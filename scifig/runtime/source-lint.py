...
Covers the skill code-gen root only:
- ``scifig/runtime/`` (helpers, generators, and related modules)

Must obey the legend contract (no in-axes ``loc="best"``,
...
def main() -> int:
    ...
    for path in iter_generator_sources():
        rel = path.relative_to(SKILL_ROOT.parents[1])
